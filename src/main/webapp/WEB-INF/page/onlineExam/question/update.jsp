...
<div class="form-content">
    <form id="form-notice-add" class="form-horizontal">
        ...
        <div class="form-group">
            <label class="col-sm-3 control-label"><span style="color: red; ">*</span>科目：</label>
            <div class="col-sm-8">
                <input type="text" style="display: none" id="subid1" value="${question.subid}">
                <select name="subid" id="subid" class="form-control" readonly>
                </select>
            </div>
        </div>
...
<script>
    ...
    $(function () {
        ...
        $.ajax({
            url:"/onlineExam/question/getSubject",    //请求的url地址
    ...
    function getData() {
        ...
        var data = {"type": type, "title": title, "score": score, "analyse": analyse, "subid": subid}
        //单选
        if (type == 1) {
    ...
    function checksubmit() {
        ...
        var analyse = $("#analyse").val();
        var subid = $("#subid").val();
        if(subid == ""){layer.msg("请选择试题科目");return false;}
        if(type == "" || type == undefined){layer.msg("请选择试题类型");return false;}
        if(title == ""){layer.msg("请输入试题");return false;}
        if(score == ""){layer.msg("请输入试题分数");return false;}
        //获取ABCD选项
        var optionA = $("input[name='optionA']").val();
        var optionB = $("input[name='optionB']").val();
        var optionC = $("input[name='optionC']").val();
        ...
        if (type == 1) {
            var radioAnswer = $("input[name='radioAnswer']:checked").val();
            if(radioAnswer == "" || radioAnswer == undefined){layer.msg("请选择答案");return false;}
            if(optionA == "" || type == undefined){layer.msg("请输入选项A");return false;}
            if(optionB == "" || type == undefined){layer.msg("请输入选项B");return false;}
            if(optionC == "" || type == undefined){layer.msg("请输入选项C");return false;}
            if(optionD == "" || type == undefined){layer.msg("请输入选项D");return false;}
        } else if (type == 2) {
            //多选
            var checkboxAnswer = $("input[name='checkboxAnswer']:checked").val();
            var s = "";
            ...
            if(s == ""){layer.msg("请选择答案");return false;}
            if(optionA == "" || type == undefined){layer.msg("请输入选项A");return false;}
            if(optionB == "" || type == undefined){layer.msg("请输入选项B");return false;}
            if(optionC == "" || type == undefined){layer.msg("请输入选项C");return false;}
            if(optionD == "" || type == undefined){layer.msg("请输入选项D");return false;}
